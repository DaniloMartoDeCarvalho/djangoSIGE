...
class GerarNotaFiscalSaidaView(CustomView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return redirect(
            reverse_lazy("fiscal:editarnotafiscalsaidaview", kwargs={"pk": nova_nota.id})
        )
...
class EmitirNotaView(CustomTemplateView):
    ...
    def emitir_nota(self):
        ...
        return redirect(
            reverse_lazy("fiscal:editarnotafiscalsaidaview", kwargs={"pk": self.object.id})
        )
    ...
    def form_valid(self, form):
        return redirect(
            reverse_lazy("fiscal:editarnotafiscalsaidaview", kwargs={"pk": self.object.pk})
        )
    ...
    def post(self, request, *args, **kwargs):
        if len(request.FILES):
            ...
            try:
                ...
                messages.error(
                    request,
                    "O seguinte erro foi encontrado ao tentar ler o arquivo XML: " + str(e),
                )
        else:
...
class GerarDanfeView(CustomView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if processador_nota.erro:
            messages.error(self.request, processador_nota.message)
            return redirect(
                reverse_lazy("fiscal:editarnotafiscalsaidaview", kwargs={"pk": nota_obj.id})
            )
        else:
            messages.success(self.request, processador_nota.message)
