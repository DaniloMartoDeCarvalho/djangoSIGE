...
class PedidoCompraEntregaHojeListView(PedidoCompraListView):
    ...
    def view_context(self, context):
        context["title_complete"] = (
            "PEDIDOS DE COMPRA COM ENTREGA DIA " + datetime.now().date().strftime("%d/%m/%Y")
        )
        context["add_url"] = reverse_lazy("compras:addpedidocompraview")
        return context
...
class GerarPedidoCompraView(CustomView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return redirect(
            reverse_lazy("compras:editarpedidocompraview", kwargs={"pk": novo_pedido.id})
        )
...
class CancelarOrcamentoCompraView(CustomView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        instance.save()
        return redirect(
            reverse_lazy("compras:editarorcamentocompraview", kwargs={"pk": instance.id})
        )
